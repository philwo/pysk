...
psql -At -F':' -c"SELECT username, substring(password from 7) as password FROM auth_user WHERE password LIKE 'crypt\$%' ORDER BY username;" pysk >> /opt/pysk/secret/htpasswd.new
